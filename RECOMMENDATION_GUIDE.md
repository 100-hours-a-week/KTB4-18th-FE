...
챗봇 입력창의 음성 버튼은 브라우저 `MediaRecorder`로 WebM 또는 MP4를 녹음합니다.
녹음은 60초에 자동 종료되며 생성 파일이 10MB를 넘으면 업로드하지 않습니다.
파일은 `POST /api/v1/speech-transcriptions`의 `audio` 필드로 전송합니다.
