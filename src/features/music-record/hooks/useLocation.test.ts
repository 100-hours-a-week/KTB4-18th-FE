...
const resolved: Location = {
  ...
  location_resolution_token: 'token',
  expires_in: 300,
};
    ...
    Object.assign(Object.create(navigator), {
      geolocation: {
        getCurrentPosition: (success: PositionCallback) => {
          onCalled();
          success({
            coords: { latitude: 37.5, longitude: 127, accuracy: 20 },
...
describe('위치 판정 토큰', () => {
  ...
  it('서버 응답 지연을 포함해 요청 시작 시각부터 300초를 계산한다', async () => {
    ...
    vi.mocked(resolveLocation).mockImplementation(
      () =>
        new Promise((resolve) => {
  ...
  it('정확한 GPS 후 역지오코딩 502는 위치 요청을 반복하지 않고 오류를 알린다', async () => {
    ...
    vi.mocked(resolveLocation).mockRejectedValue(
      new MusicApiError(502, 'reverse geocoding failed'),
    );
    const { result } = renderHook(() => useLocation());
    await act(async () => {
      await expect(result.current.acquire()).rejects.toMatchObject({ status: 502 });
    });
  ...
  it('100m 정확도를 얻지 못하면 10초 timeout 설정으로 최대 3회, 요청 사이 3초 간격 후 수동 재시도를 안내한다', async () => {
    ...
    const getCurrentPosition = vi.fn(
      (_success: PositionCallback, failure: PositionErrorCallback, options: PositionOptions) => {
    ...
    act(() => {
      request = result.current.acquire();
    });
    ...
    expect(await settled).toMatchObject({
      message: expect.stringContaining('직접 다시 시도해 주세요'),
    });
