...
import {
  getMusicRecord, updateMusicRecord,
  type MusicRecordChanges, type MusicRecordDetail,
...
export function MusicRecordDetailPage({ recordId }: MusicRecordDetailPageProps) {
  ...
  useEffect(() => {
    let active = true;
    void getMusicRecord(recordId).then((detail) => {
      if (!active) return;
      setRecord(detail);
      setPlace(detail.custom_place_name ?? '');
      setMemo(detail.emotion_memo ?? '');
    }).catch((caught: unknown) => {
      if (active) setError(caught instanceof Error ? caught.message : '기록을 불러오지 못했습니다.');
    });
    return () => { active = false; };
  }, [recordId]);
  ...
  return <main className="music-page music-detail-page">
    <header className="music-page-header">
      <a href="/music-records" aria-label="음악 기록 목록으로 돌아가기"
        onClick={(event) => { event.preventDefault(); navigate('/music-records'); }}>‹</a>
      <h1 className="text-title2-bold">기록 상세</h1><span aria-hidden="true" />
    </header>
    {error && <p role="alert" className="music-error">{error}</p>}
    {!record ? (!error && <p role="status" className="music-empty">기록을 불러오는 중…</p>)
      : <section className="music-form" aria-label="음악 기록 상세 및 수정">
        <div className="music-detail-label-row"><span>곡</span></div>
        <article className="music-selected-content">
          ...
          <div><strong>{record.music.title}</strong><span>{record.music.artist_name}</span></div>
        </article>
        <label>저장 장소<input value={`${record.region.sido.name} ${record.region.sigungu.name}`}
          readOnly aria-readonly="true" /></label>
        <label>장소 이름<input value={place} maxLength={100}
          onChange={(event) => setPlace(event.target.value)} placeholder="장소 이름 (선택)" /></label>
        <label>저장 날짜<input value={formatMusicRecordTime(record.created_at)}
          readOnly aria-readonly="true" /></label>
        <label>지금 느끼는 것 기록<textarea value={memo} maxLength={500}
          onChange={(event) => setMemo(event.target.value)} placeholder="지금의 감정을 기록해 주세요" /></label>
        <button type="button" className="music-primary-button" disabled={!hasChanges || isSaving}
          onClick={() => void save()}>{isSaving ? '저장 중…' : '저장'}</button>
      </section>}
  </main>;
}
