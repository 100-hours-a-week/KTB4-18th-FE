  ...
  it('누락되거나 중복된 유형을 전체 실패로 처리한다', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          response('terms retrieved', { items: [...items.slice(0, 5), items[0]] }),
        ),
    );
  ...
  it('상세 응답이 선택한 버전과 다르면 표시하지 않는다', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          response('term retrieved', { ...items[0], version: 'v0.4', content: 'text' }),
        ),
    );
    await expect(getTermDetail(items[0] as Parameters<typeof getTermDetail>[0])).rejects.toThrow(
      'term unavailable',
    );
  });
});
