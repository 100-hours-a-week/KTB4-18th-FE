...
function isTerm(value: unknown): value is CurrentTerm {
  ...
  return (
    Number.isSafeInteger(value.terms_id) &&
    (value.terms_id as number) > 0 &&
...
export async function getCurrentTerms(signal?: AbortSignal): Promise<CurrentTerm[]> {
  const response = await fetch(`${import.meta.env.VITE_API_BASE_URL ?? ''}/api/v1/terms`, {
    signal,
  });
  ...
  if (
    items.length !== TERM_ORDER.length ||
    !items.every(isTerm) ||
  ...
  return (items as CurrentTerm[]).sort(
    (a, b) => TERM_ORDER.indexOf(a.type) - TERM_ORDER.indexOf(b.type),
  );
}

export async function getTermDetail(term: CurrentTerm): Promise<TermDetail> {
  const response = await fetch(
    `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/v1/terms/${term.terms_id}`,
  );
  if (!response.ok) throw new Error('term unavailable');
  const body: unknown = await response.json();
  if (
    !isRecord(body) ||
    body.message !== 'term retrieved' ||
    !isRecord(body.data) ||
    !isTerm(body.data) ||
    body.data.terms_id !== term.terms_id ||
    body.data.type !== term.type ||
